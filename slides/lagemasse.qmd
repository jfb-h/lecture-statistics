...
-   Das *arithmetische Mittel* fasst eine Datenreihe in einer Zahl zusammen, die deren "Mitte" bzw.
    "zentrale Tendenz" darstellen soll.
...
Das arithm. Mittel ist definiert als die Summe der Werte geteilt durch die Anzahl der Werte. Für die
folgende Datenreihe $x$ mit $n=4$ Werten:
...
-   Der Laufindex $\color{green}{i}$ indiziert die Werte in $x$, startet bei $\color{green}{i=1}$
    und läuft bis $\color{blue}{n}$ (in Schritten von 1).
...
Im Fall einer Dummyvariable (die die Werte 0 oder 1 annehmen kann), gibt das arithmetische Mittel
eine Proportion an:
...
-   Bei Verhältnisskalen (Zahlen, die in einem multiplikativen Verhältnis zueinander stehen), sollte
    das *geometrische* und nicht das *arithmetische* Mittel eingesetzt werden.
...
::: {.column width="35%"}
Die Werte zeigen das *Verhältnis* des diesjährigen BIP zum Vorjahreswert an, d.h. eine
Wachstumsrate.
...
Analog zur Summe im arithm. Mittel können wir das geom. Mittel mit der Produktnotation ($\Pi$, groß
Pi) definieren:
...
-   In vielen Situationen muss z.B. für Größenunterschiede der Beobachtungseinheiten korrigiert
    werden.
-   In solchen Fällen können Werte für das geometrische oder arithmetische Mittel individuell
    gewichtet werden.

## [D]{.sq .blue} Gewichtetes arithmetisches Mittel

Das gewichtete arithmetische Mittel ist die Summe der gewichteten Werte geteilt durch die Summe der
Gewichte.
...
Die Anzahl der Werte $n$ wird wie beim gewichteten arithm. Mittel durch die Summe der Gewichte
ersetzt.
...
-   Der Median berücksichtigt ausschließlich die Reihenfolge (den Rang) der Werte zur Identifikation
    der 'Mitte'.
-   Der Median ist robuster, tauscht dafür aber Informationsgehalt ein.

## [D]{.sq .blue} Der Median

-   Der Median ist der Wert in der Mitte einer größensortierten Datenreihe. Bei einer geraden Anzahl
    an Werten $n$ ist der Median das arithmetische Mittel der beiden mittleren Werte.
...
Beispiel mit $n$ ungerade: $$s = {163, 177, \color{orange}{179}, 180, 192}$$
$$s_\color{orange}{\frac{n + 1}{2}} = 179$$

Beispiel mit $n$ gerade: $$s = {163, \color{orange}{179}, \color{orange}{180}, 192}$$
...
-   Der Median ist das 0,5-Quantil bzw. das 50%-Perzentil, d.h. 50% der Werte sind kleiner als der
    Median.
-   Wir können andere Quantile berechnen, z.B. um eine Aussage über einen "Großteil" (z.B.
    90%-Perzentil) zu treffen.
